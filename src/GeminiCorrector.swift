...
final class GeminiCorrector: GrammarCorrector {
  ...
  private func makePrompt(text: String, attempt: Int) -> String {
    ...
    if attempt > 1 {
      lines.insert("IMPORTANT: Your previous output changed the text too much. This time, keep everything identical except for the minimal characters needed to correct errors.", at: 2)
    }
